...
class Vonage implements GatewayInterface {
	...
	public function send( $form_data, $options ) {
		...
		if ( '' === $api_key || '' === $api_secret ) {
			return new WP_Error( 'no-gateway-settings', __( 'No API key or Secret found', 'wp-form-sms' ), [ 'status' => 401 ] );
		}
		...
		if ( $message->getStatus() == 0 ) {
			...
			$response = [
				'message'  => __( 'The message failed with status:', 'wp-form-sms' ) . $message->getStatus(),
				'response' => $message,
			];
			return $response;
		}
